...
class NMRRetargetService:
    ...
    def retarget_chunk(self, chunk: NMRInputChunk) -> dict[str, Any]:
        ...
        Mirrors infer_single() but starts from a tensor instead of a file.
        Canonical alignment + standardization are done once for the whole
        motion, then model forward runs in chunks with overlap blending.
        """
        self._load()
        _add_nmr_path()
        ...
        # Canonical alignment (align first frame's yaw to zero)
        ...
        # Standardize once for whole motion
